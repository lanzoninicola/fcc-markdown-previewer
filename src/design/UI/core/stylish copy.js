...
function handleCalculatedRule() {}

function handleStandardRule(stylesheetRule = {}) {
  const { name, value } = stylesheetRule;

  const hyphenatedName = hyphenateStyleName(name);

  return {
    ruleName: hyphenatedName,
    ruleValue: value,
  };
}
...
function handleStylesheetPrivateRule(selectorName, componentName, stylesheetRule = {}) {

  console.log(stylesheetRule)

  //  name: "_hover2", value: {backgroundColor: "red"}

   // _pseudoClasses
   if(isPseudoclass(stylesheetRule.name)) {
    const pseudoClass = getPseudoclass(stylesheetRule.name)

    // console.log(pseudoClass) prototype: "#:hover", description: "selects links on mouse over"}
    
    const newSelectorName = pseudoClass.prototype.replace(/#/, selectorName)
    console.log(newSelectorName)

    return {
      [`${selectorName}${stylesheetRule.name}`]: {
          ...stylesheetRule.value
      }
      // ,nameText: newSelectorName,
    }
  }

  // _children
  // manage if it is an object (child style => "Parent > child" )



  // return
          // {
        //   flexColumnCenter:hover: {
        //       backgroundColor: "red"
        //   }
        // }
}

function handleStylesheetRule(stylesheetRule = {}) {
  let selectorRuleName = "";
  let selectorRuleValue = "";
  let selectorRuleText = "";

  //*** The CSS object prop value is a string ***//
    if (typeof stylesheetRule.value === "string") {
      const { ruleName, ruleValue } = handleStandardRule(stylesheetRule);
      selectorRuleName = ruleName;
      selectorRuleValue = ruleValue;
      selectorRuleText = `${ruleName}: ${ruleValue}; `;
    }
    //*** The CSS object prop value is a function due manage theme functions ***//
    // ((color) => fn(color))
    if (typeof stylesheetRule.value === "function") {
      handleCalculatedRule();
    }

  return {
    ruleName: selectorRuleName,
    ruleValue: selectorRuleValue,
    ruleText: selectorRuleText,
  };
}
...
function handleStylesheetRuleList(stylesheet = {}) {
  const { name, componentName } = stylesheet;
  
  let stylesheetRuleList = {}
  let rule = {}

  stylesheetRuleList = {
    ...stylesheetRuleList,
    [selectorName]: {
      stylesheetNameText: `${selectorName}`,
      stylesheetRulesText: "",
      rules: {
        ...rule
      }
  ...
  Object.keys(stylesheet["rules"]).forEach((rule) => {
    
    const stylesheetRule = {
      name: rule,
      value: stylesheet["rules"][rule],
    };


    // manage if it is an private key with underscore (eg. _hover)
    if (isPrivateKey(rule)) {
        const stylesheetsObject = handleStylesheetPrivateRule(selectorName, componentName, stylesheetRule)
        ...
        handleStylesheetList(stylesheetsObject, componentName)
    
    // otherwise is a regular rule (eg. display: flex)
    } else {


      const { ruleName, ruleValue, ruleText } = handleStylesheetRule(
        stylesheetRule
      );
      ...
      stylesheetRuleList[selectorName]["rules"][ruleName] = ruleValue;
      stylesheetRuleList[selectorName]["stylesheetRulesText"] += ruleText.trim();

    }
  });

 
  return stylesheetRuleList;
}

function buildStylesheetName(name, componentName) {
  return componentName !== "" ? `${componentName}__${name}` : name;
}

function handleStylesheetList(stylesheetsObject = {}, componentName = "") {
  console.time("handleStylesheetList");

  let { cssRules } = appStylesheets;
  let stylesheetsRules={}
  ...
  console.timeEnd("handleStylesheetList");
}
